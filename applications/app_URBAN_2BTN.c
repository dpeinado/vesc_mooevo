...
// Private variables specific this application

static const volatile mc_configuration *mc_conf;
mc_configuration *mc_conf_mod;

typedef struct {
  float erpmM_marchaAtras;  // maximum erpms reverse
  float erpmM_andando;      // maximum erpm  walking
  float erpmM_vehiculo;     // maximum erpm  as a vehicle
  float max_omega;          // maximum value of angular acceleration
  float omega_cut;          // When acceleration begins to cut off
  float k_filter;           // Acceleration Filter (0 maximum filtration 1 no filter)
} urban_config;

urban_config uconf;


// convert kilometers per hour to electric rpm
...
void app_custom_configure(app_configuration *conf) {
    commands_printf("Entrando en CONFIGURE");
	...
	commands_printf("Vehículo: %f", (double) uconf.erpmM_vehiculo);
}
...
static THD_FUNCTION(urban_thread, arg) {
	...
	for(;;) {
		...
		// lectura del voltaje del acerlerador
		...
		bool veh_button = false;
		bool rev_button = false;

		veh_button = palReadPad(HW_UART_TX_PORT, HW_UART_TX_PIN);
        ...
        // Se calculan los valores mínimos (marcha atrás) y máximos (marcha alante) de rpms en función del modo elegido
        if (rev_button) {
          if (veh_button){ // marcha atrás y modo vehículo
            max_rpm_conf = 0.0;
            min_rpm_conf = - uconf.erpmM_marchaAtras;
          } else { // marcha atrás y modo andarín
            max_rpm_conf = 0.0;
            min_rpm_conf = 0.0;
          }
        } else{
          if (veh_button){ // marcha adelante y modo vehículo
            max_rpm_conf = uconf.erpmM_vehiculo;
            min_rpm_conf = 0.0;
          } else { // marcha adelante y modo andarín
            max_rpm_conf = uconf.erpmM_andando;
            min_rpm_conf = 0.0;
          }
        }
        // aplico los máximos a la corriente de salida en función del modo seleccionado
        if (rpm_avg >= 0) {
        ...
        // controlo que la aceleración no supere un valor máximo almacenado en app_balance_conf.kp2;
        float omega = 0.0;
